...
<form name="myedit" id="myedit" action="[{$oViewConf->getSelfLink()}]" method="post" style="">
    ...
    <fieldset style="padding: 5px; margin-bottom: 10px; height: 125px; width: 65%; float: left;">
        <legend><b><label for="newmoditems">[{oxmultilang ident="D3_CFG_MODITEM_NEWITEMS"}]</label></b></legend>
        <textarea cols="150" rows="5" name="newmoditems" id="newmoditems" style="border: none; width: 99%;">[{$newMods}]</textarea>
    </fieldset>
    ...
    <div class="d3modcfg_btn fixed icon d3color-blue">
        <button type="submit" name="save">
            <i class="fa fa-question-circle fa-inverse"></i>[{oxmultilang ident="D3_CFG_MODITEM_PREVIEW"}]
        </button>
    </div>

    <div class="d3modcfg_btn fixed icon d3color-green" style="margin-left: 10px;">
        <button type="submit" name="save" onclick="document.getElementById('myedit').fnc.value='setNewMods'; return;">
            <i class="fa fa-check-circle fa-inverse"></i>[{oxmultilang ident="D3_CFG_MODITEM_SAVE"}]
        </button>
    </div>
            ...
            [{foreach from=$aModPreview key="sClassKey" item="aClassExtension"}]
                <tr>
                    ...
                    <td style="vertical-align: top;">
                        [{if isset($aDisabledModules[$sClassKey]) && is_array($aDisabledModules[$sClassKey]) && in_array($sModule, $aDisabledModules[$sClassKey])}]
