...
[{if $oView->isWriteable()}]
    <div class="d3modcfg_btn fixed icon d3color-green" style="margin-left: 10px;">
        <button type="submit" name="save">
            <i class="fa fa-check-circle fa-inverse"></i>[{if $oView->hasRequiredWriteProtection()}][{oxmultilang ident="D3_CFG_CFGITEM_SAVE"}][{else}][{oxmultilang ident="D3_CFG_CFGITEM_SAVEWOPROTECTION"}][{/if}]
        </button>
    </div>
[{else}]
    <div class="d3modcfg_btn fixed icon d3color-red" style="margin-left: 10px;">
        <button type="submit" name="save">
            <i class="fa fa-exclamation-triangle fa-inverse"></i>[{oxmultilang ident="D3_CFG_CFGITEM_EDIT"}]
        </button>
    </div>
[{/if}]
