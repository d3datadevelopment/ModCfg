...
<fieldset style="padding: 5px; margin-bottom: 10px; height: 91%; width: 99%; float: left; background-color: white;">
    ...
    <form name="myedit" id="myedit" action="[{$oViewConf->getSelfLink()}]" method="post" style="height: 97%;" [{if !$oView->isWriteable()}]onSubmit="return confirm('[{oxmultilang ident="D3_CFG_CFGITEM_EDITABLE_QUESTION"}]');"[{else}]onSubmit="return confirm('[{oxmultilang ident="D3_CFG_CFGITEM_SAVE_QUESTION"}]');"[{/if}]>
        ...
        [{/if}]
        <textarea cols="150" rows="5" name="newcfg" style="font-family: Courier, monospace; border: none; width: 100%; height: 100%; [{if !$oView->isWriteable()}]color: silver;[{/if}]" [{if !$oView->isWriteable()}]readonly [{/if}]>[{$sCfgContent}]</textarea>
</fieldset>

[{if $oView->isWriteable()}]
    <div class="d3modcfg_btn fixed icon d3color-green" style="margin-left: 10px;">
        <button type="submit" name="save">
            <i class="fas fa-check-circle fa-inverse"></i>[{if $oView->hasRequiredWriteProtection()}][{oxmultilang ident="D3_CFG_CFGITEM_SAVE"}][{else}][{oxmultilang ident="D3_CFG_CFGITEM_SAVEWOPROTECTION"}][{/if}]
        </button>
    </div>
[{else}]
    <div class="d3modcfg_btn fixed icon d3color-red" style="margin-left: 10px;">
        <button type="submit" name="save">
            <i class="fas fa-exclamation-triangle fa-inverse"></i>[{oxmultilang ident="D3_CFG_CFGITEM_EDIT"}]
        </button>
    </div>
[{/if}]
