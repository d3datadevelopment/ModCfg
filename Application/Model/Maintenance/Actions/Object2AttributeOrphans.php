...
class Object2AttributeOrphans extends AbstractAction
{
    /**
     * @return int
    ...
    protected function getBaseQuery(string $tableAlias): QueryBuilder
    {
        ...
        $oQB->from('oxobject2attribute', $tableAlias)
            ->leftJoin($tableAlias, 'oxarticles', 'art', $oQB->expr()->eq($tableAlias.'.oxobjectid', 'art.oxid'))
            ->leftJoin($tableAlias, 'oxattribute', 'obj', $oQB->expr()->eq($tableAlias.'.oxattrid', 'obj.oxid'))
            ->where(
                $oQB->expr()->or(
